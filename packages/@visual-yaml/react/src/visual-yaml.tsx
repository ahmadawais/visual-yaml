...
import { type StudioActions, StudioContext, type StudioState } from "./context";
import { computeRangeIds } from "./selection-utils";

export interface VisualYamlProps {
	value: YamlValue;
...
export function VisualYaml({
	value,
	onChange,
	schema,
	children,
}: VisualYamlProps) {
	const [tree, setTreeState] = useState<TreeState>(() => fromYaml(value));
	...
	const nextSearchMatch = useCallback(() => {
		...
		if (match) focusSelectAndDrillDown(match.nodeId);
