...
import type { NodeType, TreeNode } from "@visual-yaml/core";
import {
	changeType,
	duplicateNode,
	removeNode,
...
import { computeSelectAllIds, deleteSelectedNodes } from "./selection-utils";
import {
	type DragState,
	setMultiDragImage,
	useDragDrop,
				...
				case "ArrowRight": {
					...
					if (node && (node.type === "object" || node.type === "array")) {
						if (!state.expandedNodeIds.has(node.id)) {
							...
							if (first) actions.selectNode(first.id);
						}
					}
					break;
				}
