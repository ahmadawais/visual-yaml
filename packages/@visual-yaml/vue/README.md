...
Vue 3 UI components for [visual-yaml](https://github.com/vercel-labs/visual-yaml) — the visual YAML editor. Schema-aware, embeddable, extensible.
...
```bash
npm install @visual-yaml/vue @visual-yaml/core
```
...
<script setup>
import { ref } from "vue";
import { YamlEditor } from "@visual-yaml/vue";

const value = ref({ hello: "world" });
</script>

<template>
  <YamlEditor :value="value" @change="value = $event" />
</template>
```
...
```vue
<script setup>
import { VisualYaml, TreeView, FormView, SearchBar } from "@visual-yaml/vue";
</script>
...
See the [default variable list](src/components/YamlEditor.vue) for all available tokens.
