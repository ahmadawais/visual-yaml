...
describe("setValue", () => {
	it("updates a primitive value", () => {
		...
		expect(toYaml(next.root)).toEqual({ a: 42 });
	});
...
describe("setKey", () => {
	...
	it("does not rename array item keys", () => {
		const state = fromYaml([1, 2]);
		const child = state.root.children[0];
		const next = setKey(state, child.id, "notAllowed");
		expect(toYaml(next.root)).toEqual([1, 2]);
	});
});
...
describe("addProperty", () => {
	it("adds to an object", () => {
		...
		expect(toYaml(next.root)).toEqual({ a: 1, b: 2 });
	});

	it("appends to an array", () => {
		const state = fromYaml([1, 2]);
		const next = addProperty(state, state.root.id, "2", 3);
		expect(toYaml(next.root)).toEqual([1, 2, 3]);
	});
...
describe("moveNode", () => {
	...
	it("moves within the same parent at a specific index", () => {
		...
		expect(toYaml(next.root)).toEqual([30, 10, 20]);
	});
});
...
describe("reorderChildren", () => {
	it("reorders object keys", () => {
		...
		expect(Object.keys(toYaml(next.root) as Record<string, unknown>)).toEqual([
			"b",
			"c",
			"a",
		]);
...
describe("changeType", () => {
	it("converts a string to a number", () => {
		const state = fromYaml({ val: "42" });
		const child = state.root.children[0];
		const next = changeType(state, child.id, "number");
		expect(toYaml(next.root)).toEqual({ val: 42 });
	});
	...
	it("converts a primitive to an array", () => {
		const state = fromYaml({ val: 5 });
		const child = state.root.children[0];
		const next = changeType(state, child.id, "array");
		expect(toYaml(next.root)).toEqual({ val: [5] });
	});

	it("converts to null", () => {
		const state = fromYaml({ val: "hello" });
		const child = state.root.children[0];
		const next = changeType(state, child.id, "null");
		expect(toYaml(next.root)).toEqual({ val: null });
	});
});
...
describe("duplicateNode", () => {
	it("duplicates an object property", () => {
		...
		const yaml = toYaml(next.root) as Record<string, unknown>;
		expect(yaml).toEqual({ a: 1, a_copy: 1, b: 2 });
	});
	...
	it("duplicates an array item", () => {
		...
		expect(toYaml(next.root)).toEqual([10, 10, 20]);
	});
...
describe("insertProperty", () => {
	...
	it("inserts at a middle index", () => {
		...
		expect((toYaml(next.root) as Record<string, unknown>)["mid"]).toBe(99);
	});
	...
	it("inserts into an array and re-indexes keys", () => {
		...
		expect(toYaml(next.root)).toEqual([10, 15, 20, 30]);
...
describe("reorderChildrenMulti", () => {
	it("moves multiple adjacent children after a target", () => {
		const state = fromYaml({ a: 1, b: 2, c: 3, d: 4 });
	...
	it("reorders within an array and re-indexes keys", () => {
		const state = fromYaml([10, 20, 30, 40]);
		...
		expect(toYaml(next.root)).toEqual([30, 40, 10, 20]);
		expect(next.root.children.map((c) => c.key)).toEqual(["0", "1", "2", "3"]);
	});
...
describe("insertNode", () => {
	it("inserts a node while preserving its ID", () => {
		const src = fromYaml({ src: { val: 1 }, dst: {} });
	...
	it("preserves descendant IDs", () => {
		const src = fromYaml({ src: { obj: { a: 1, b: 2 } }, dst: {} });
	...
	it("re-indexes array keys when inserting into an array", () => {
		const state = fromYaml({ items: [10, 20, 30], extra: 99 });
		...
		expect(toYaml(next.root)).toEqual({ items: [10, 99, 20, 30] });
	...
	it("registers inserted nodes in nodesById", () => {
		const src = fromYaml({ src: { obj: { deep: 1 } }, dst: {} });
		const obj = src.root.children[0].children[0];
	...
	it("works correctly with structuredClone'd nodes after removal", () => {
		const state = fromYaml({ src: { a: 1, b: 2 }, dst: {} });
		...
		expect(toYaml(tree.root)).toEqual({ src: {}, dst: { a: 1, b: 2 } });
	...
	it("cross-parent multi-move preserves IDs with structuredClone", () => {
		const state = fromYaml({ src: { obj: { x: 1 } }, dst: { existing: 0 } });
		...
		expect(toYaml(next.root)).toEqual({
			src: {},
