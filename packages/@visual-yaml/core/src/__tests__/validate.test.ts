...
import { findNodeByPath, fromYaml, resetIdCounter } from "../tree";
import type { YamlSchemaProperty } from "../types";
...
function nodeAt(value: unknown, path: string) {
	const state = fromYaml(value as never);
...
describe("validateNode", () => {
	...
	it("detects type mismatch", () => {
		...
		const schema: YamlSchemaProperty = { type: "number" };
	...
	it("validates union types", () => {
		const node = nodeAt({ a: "hello" }, "/a");
		const schema: YamlSchemaProperty = { type: ["string", "number"] };
		expect(validateNode(node, schema).valid).toBe(true);
	});

	it("treats number as integer", () => {
		const node = nodeAt({ a: 5 }, "/a");
		const schema: YamlSchemaProperty = { type: "integer" };
		expect(validateNode(node, schema).valid).toBe(true);
	});

	it("validates enum values", () => {
		const node = nodeAt({ a: "baz" }, "/a");
		const schema: YamlSchemaProperty = { enum: ["foo", "bar"] };
		const result = validateNode(node, schema);
		expect(result.valid).toBe(false);
	...
	it("validates const", () => {
		...
		const schema: YamlSchemaProperty = { const: 1 };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates required properties", () => {
		const node = nodeAt({ obj: { a: 1 } }, "/obj");
		const schema: YamlSchemaProperty = { required: ["a", "b"] };
		const result = validateNode(node, schema);
		expect(result.valid).toBe(false);
	...
	it("validates minimum", () => {
		...
		const schema: YamlSchemaProperty = { type: "number", minimum: 5 };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates maximum", () => {
		const node = nodeAt({ a: 10 }, "/a");
		const schema: YamlSchemaProperty = { type: "number", maximum: 5 };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates exclusiveMinimum", () => {
		const node = nodeAt({ a: 5 }, "/a");
		const schema: YamlSchemaProperty = {
			type: "number",
			exclusiveMinimum: 5,
		};
		expect(validateNode(node, schema).valid).toBe(false);
	...
	it("validates multipleOf with integers", () => {
		...
		const schema: YamlSchemaProperty = { type: "number", multipleOf: 3 };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates multipleOf with floats correctly", () => {
		const node = nodeAt({ a: 0.3 }, "/a");
		const schema: YamlSchemaProperty = { type: "number", multipleOf: 0.1 };
		expect(validateNode(node, schema).valid).toBe(true);
	});

	it("validates minLength", () => {
		const node = nodeAt({ a: "ab" }, "/a");
		const schema: YamlSchemaProperty = { type: "string", minLength: 5 };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates maxLength", () => {
		const node = nodeAt({ a: "abcdef" }, "/a");
		const schema: YamlSchemaProperty = { type: "string", maxLength: 3 };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates pattern", () => {
		const node = nodeAt({ a: "hello" }, "/a");
		const schema: YamlSchemaProperty = { type: "string", pattern: "^\\d+$" };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates format (email)", () => {
		const node = nodeAt({ a: "not-an-email" }, "/a");
		const schema: YamlSchemaProperty = { type: "string", format: "email" };
		expect(validateNode(node, schema).valid).toBe(false);
	});
	...
	it("validates minItems", () => {
		...
		const schema: YamlSchemaProperty = { type: "array", minItems: 3 };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates maxItems", () => {
		const node = nodeAt({ arr: [1, 2, 3, 4] }, "/arr");
		const schema: YamlSchemaProperty = { type: "array", maxItems: 2 };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates minProperties", () => {
		const node = nodeAt({ obj: { a: 1 } }, "/obj");
		const schema: YamlSchemaProperty = { type: "object", minProperties: 3 };
		expect(validateNode(node, schema).valid).toBe(false);
	});

	it("validates maxProperties", () => {
		const node = nodeAt({ obj: { a: 1, b: 2, c: 3 } }, "/obj");
		const schema: YamlSchemaProperty = { type: "object", maxProperties: 2 };
		expect(validateNode(node, schema).valid).toBe(false);
	});
});
