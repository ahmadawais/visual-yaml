...
import { getAncestorIds, searchNodes } from "../search";
import { fromYaml, resetIdCounter } from "../tree";
...
describe("searchNodes", () => {
	it("returns empty for blank query", () => {
		const state = fromYaml({ name: "Alice" });
		expect(searchNodes(state, "")).toEqual([]);
		expect(searchNodes(state, "   ")).toEqual([]);
	});

	it("matches keys", () => {
		const state = fromYaml({ username: "alice", email: "a@b.com" });
...
describe("getAncestorIds", () => {
	...
	it("merges ancestors for multiple nodes", () => {
		const state = fromYaml({ a: { x: 1 }, b: { y: 2 } });
