...
import { getPropertySchema, resolveRef } from "../schema";
import type { YamlSchemaProperty } from "../types";

describe("resolveRef", () => {
	it("resolves a simple $ref", () => {
		const root: YamlSchemaProperty = {
			definitions: {
				Foo: { type: "string", description: "A foo" },
			},
		};
		const prop: YamlSchemaProperty = { $ref: "#/definitions/Foo" };
	...
	it("handles missing $ref gracefully", () => {
		const root: YamlSchemaProperty = {};
		const prop: YamlSchemaProperty = { $ref: "#/definitions/Missing" };
		const resolved = resolveRef(prop, root);
	...
	it("prevents infinite cycles", () => {
		...
		const prop: YamlSchemaProperty = { $ref: "#/definitions/A" };
		const resolved = resolveRef(prop, root);
