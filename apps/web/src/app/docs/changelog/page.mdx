...
- **Vue support** — new `@visual-yaml/vue` package with `VisualYaml`, `YamlEditor`, `TreeView`, `FormView`, `DiffView`, `SearchBar`, `Breadcrumbs`, and `ContextMenu` components for Vue 3.

### Internal

- Refactored monorepo package structure: packages moved under `packages/@visual-yaml/` namespace.
...
- `isDescendant` utility moved to `@visual-yaml/core` with unit tests.
...
- YAML-to-tree model (`fromYaml` / `toYaml`) with stable node IDs.
...
- YAML schema resolution and per-node validation (`resolveSchema`, `validateNode`).
